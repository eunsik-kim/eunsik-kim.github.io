...
결국 늘 그랬듯이 vine test를 하며 debugging하기 전까지 틀린줄도 모르고 계속 사용했었습니다.
...
ASSERT를 넣어 debbuing을 해보면 항상 cluster_to_sector에서 많이 에러가 발생했습니다. 그래도 어디부터 chain이 망가진건지 추측하기는 쉽지 않습니다. 

> fat와 free-map의 booting 과정은 같으면서도 다릅니다. 간단히 보면 fat_init() -> do_format -> fat_open 순서로 동일하게 진행됩니다. 1.system을 초기화 한뒤 2. system을 create하고 3. close하면서 disk에 내용을 기록하고 4.마지막으로 다시 open하여 fat가 시작이 됩니다. 다만 중요한점은 기존에 system은 root dir을 열어주지만 fat는 열어주지 않기에 따로 추가해줘야합니다. (이 내용이 gitbook에 없는점, root를 안열고 0으로 memset만 해주는 점은 이해가 되지 않습니다.)
{: .prompt-info }

그리고 file grow를 구현해야 합니다. 조건문을 해결하기 위해 10시간 이상을 debbuging에 했기 때문에 좀 이상하더라도 양해 부탁드립니다. 
...
동시에 읽어 i/o 처리 효율 향상을 위해 block은 최대한 적게 하도록 노력했습니다. 구체적으로는 syn_read test와 syn-rw test를 통과하도록 inode write에서만 lock을 걸고 확인하도록 했습니다. file growth하는 경우에만 일관성을 보장받아야 하기에 filesys lock을 제거하고 inode에 의존하는 lock을 걸었습니다. 
...
같은 directory혼자서 진입해야 한다고 하였기에 directory에 종속적인 lock을 만들어 dir 내용을 확인하는 함수에 dir_readdir, dir_remove, dir_add에 lock을 걸었습니다.
...
sub directory를 구현하는데 가장 고민했던 부분은 path를 parsing하여 효율적으로 다른 함수들을 활용 할 수 있는 방법을 찾아내는 것이였습니다. 그래서 그냥 parsing을 먼저 하다보니 위와 같은 함수를 만들 수 있었습니다. 가장 주의해야 할 부분은 overflow가 생기지 않도록 malloc을 하고, 입력으로 주어지는 path를 손상시키지 않도록 복사하여 활용하는 것입니다.   

overflow를 처음엔 인지못하여서 나중에 힘들게 찾는 고생을 하지 않도록 주의를 해야합니다. 위의 함수를 만들고 나면 filesys에 있는 dir open root를 모두 위의 함수로 바꾸고 syscall의 함수들을 만들면 쉽게 할 수 있습니다. 
...
하지만 sym_link test에선 아직 생성되지 않은 file까지 directing하고 있음을 알 수 있습니다.... 그래서 path을 저장하여 관리하였고 참조시 해당 파일을 찾아 바뀌도록 구현하였습니다.  
